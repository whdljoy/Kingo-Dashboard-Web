...
app.get('/api/mapping', (req, res) => {
  ...
  connection.query(`SELECT * from mapping where skkuid=${skkuid}`, (err, rows, fields) => {
    if (err) throw err;
    res.json(rows);
  });
});
...
app.post('/api/createTx', async (req, res) => {
  ...
  connection.query(`SELECT _username, _point${_type} from user where _username = "${_from}" or _username="${_to}"`, (err, rows) => {
    if (err) throw err;
    const variable1 = rows[0][Object.keys(rows[0])[1]];
    const variable2 = rows[1][Object.keys(rows[1])[1]];

    rows[0][Object.keys(rows[0])[0]] == _from ? ((fromPoint = variable1), (toPoint = variable2)) : ((fromPoint = variable2), (toPoint = variable1));
...
app.get('/api/checkPointsFrom', (req, res) => {
  ...
  connection.query(`SELECT _point${type} from user where _username = "${_from}"`, (err, rows) => {
    if (err) throw err;
    res.json(rows[0]);
  });
});
...
app.get('/api/userinfo', (req, res) => {
  console.log(req);
  connection.query(`SELECT * from user where _username="${req.query.id}"`, (err, rows, fields) => {
    if (err) throw err;
    res.json(rows);
  });
});
...
app.post('/api/tester', (req, res) => {
  ...
  connection.query(`UPDATE USER set _point${type}=_point${type}-${point} where _username="${_from}"`, (err, rows) => {
    if (err) throw err;
    res.json(rows);
  });
