import { Flex, HStack, Text, VStack, Box, Input, Button, FormControl, FormLabel, FormErrorMessage, FormHelperText } from '@chakra-ui/react';
...
function AdminPage() {
  ...
  const handleCreateTransaction = () => {
    axios.post('http://localhost:5000/api/createTx', {
      _from: from,
      _to: to,
      point: point,
      type: type,
    ...
    <Flex m={10} flexDirection="column" w="full">
      ...
      <VStack w="full" backgroundColor="white" borderRadius="5px" p={5} marginTop={5}>
        ...
        <FormControl id="transaction-create">
          ...
          <Input placeholder="ex) Alice" onChange={(e) => setFrom(e.target.value)} />
          <FormLabel>To</FormLabel>
          <Input placeholder="ex) Bob" onChange={(e) => setTo(e.target.value)} />
          <FormLabel>Point</FormLabel>
          <Input placeholder="ex) 300" onChange={(e) => setPoint(e.target.value)} />
          <FormLabel>Type</FormLabel>
          <Input placeholder="ex) A" onChange={(e) => setType(e.target.value)} />
          <Button
            type="submit"
      ...
      <VStack w="full" backgroundColor="white" borderRadius="5px" p={5} marginTop={5} align="flex-start">
        <HStack justifyContent="space-between" w="full">
          <Text color="#4318FF" fontWeight={700} fontSize="lg">
            Demo
          </Text>
        ...
        <WalletConnect />
        <Button type="submit" onClick={handleDemo} h="40px" borderRadius={20} w="200px" bg="#4318FF" color="white" fontSize="sm" fontWeight="bold" mt={5}>
          <Text>Demo 실행</Text>
        </Button>
      </VStack>
    </Flex>
